...
my $outfasta = $opts{output}."/eRParranger_tmp.fasta";
...
my $inputfasta = $opts{output}."/contig_tmp.fasta";
open(APRES, "> $outappres");
open(ICOV, "> $inputcov");
open(IFASTA, "> $inputfasta");
print IFASTA ">input_tmp\n";
for my $contig (sort {$sam{$b}{L} <=> $sam{$a}{L}} keys %sam){
    next unless $used{$contig};
    print IFASTA $contigs{$contig}{SEQ}."\n";
    for my $pos (sort {$a <=> $b} keys %{$sam{$contig}}){
	next if $pos eq "L";
	push(@{$coverage{$contig}{pos}}, $pos);
	push(@{$coverage{$contig}{cov}}, $sam{$contig}{$pos});
...
sub approximation{
    ...
    my $ave_x = $sum_x / $num;
    my $slope;
    if($sum_xx - $num * $ave_x * $ave_x){
	$slope = ($sum_xy - $num * $ave_y * $ave_x) / ($sum_xx - $num * $ave_x * $ave_x);
    }else{
	$slope = 0;
    }
